...
class AuthService(
    ...
    suspend fun create(authInfo: AuthInfo) = withContext(dispatcherProvider.io()) {
        getConnection().use { connection ->
            connection.prepareStatement(INSERT_AUTH, Statement.RETURN_GENERATED_KEYS).use { statement ->
    ...
    suspend fun update(authInfo: AuthInfo) = withContext(dispatcherProvider.io()) {
        getConnection().use { connection ->
            connection.prepareStatement(UPDATE_AUTH).use { statement ->
    ...
    suspend fun delete(userId: Long) = withContext(dispatcherProvider.io()) {
        getConnection().use { connection ->
            connection.prepareStatement(DELETE_AUTH).use { statement ->
                statement.setLong(1, userId)
                statement.executeUpdate()
            }
        }
